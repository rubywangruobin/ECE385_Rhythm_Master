...
BYTE SPI_wr(BYTE data) {
	...
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, data, 1, &status_reg, 0);
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	pass[1] = val;
	//write val via SPI
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, pass, 0, NULL, 0);
	if(return_code < 0){
		alt_printf("ERROR SPI TX RET = %x \n", return_code);
	}
	//read return code from SPI peripheral (see Intel documentation) 
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes+1, pass, 0, NULL, 0);
	if(return_code < 0){
		alt_printf("ERROR SPI TX RET = %x \n", return_code);
	}
	//read return code from SPI peripheral (see Intel documentation) 
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE val;
	int return_code;
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, potato, 2, &val, 0);
	if(return_code < 0){
		alt_printf("ERROR SPI TX RET = %x \n", return_code);
	}
	return val;
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int return_code;
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, potato, nbytes+1, data, 0);
	if(return_code < 0){
		alt_printf("ERROR SPI TX RET = %x \n", return_code);
	}
	//read return code from SPI peripheral (see Intel documentation)
